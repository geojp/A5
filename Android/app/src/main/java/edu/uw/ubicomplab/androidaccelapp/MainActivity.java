...
public class MainActivity extends AppCompatActivity implements BluetoothLeUart.Callback {
    ...
    private static final String DEVICE_NAME = "SomeDevice";
    ...
    {
        if (recvBuffer.startsWith("FStart") && recvBuffer.indexOf("FStop") != -1)
        {
            // we have a full feature vector
            String[] feats = recvBuffer.split(" ");
            feats = Arrays.copyOfRange(feats, 1, feats.length - 1);
            model.addFeatures(feats, recentLabel, isRecentTrain);
            if (!isRecentTrain) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
    ...
    public void onReceive(BluetoothLeUart uart, BluetoothGattCharacteristic rx) {
        ...
        recvBuffer += rx.getStringValue(0);
